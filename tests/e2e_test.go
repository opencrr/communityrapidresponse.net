...
func SetupE2ETest(t *testing.T) *E2ETestSuite {
	...
	groupHandler := handlers.NewGroupHandler(db, communityGroupRepo, groupRepo, meshtasticChannelRepo, regionRepo, userRepo, auditRepo, encryptedSecretRepo)
	connectionHandler := handlers.NewConnectionHandler(connectionRepo, communityGroupRepo, auditRepo, encryptedSecretRepo, userRepo)
