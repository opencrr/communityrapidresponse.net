...
3. **Vouch Verification Request**: User requests vouch verification by entering their city/state (text input) → system matches to existing region → creates pending `user_regions` entry → user connects with fully verified neighbors (out-of-band) → 2 different fully verified users from same region vouch via app → system validates (10/month limit, no circular patterns, same region) → user gains vouch-verified status (read-only access, or admin access if already postcard-verified)
