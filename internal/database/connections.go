...
func (r *ConnectionRepository) LeaveConnection(ctx context.Context, connectionID, groupID string) error {
	return r.db.Transaction(ctx, func(tx *sql.Tx) error {
		...
		if err := secretRepo.RevokeConnectionSecretKeysForGroup(ctx, tx, connectionID, groupID); err != nil {
			return err
		}
...
// ListConnectionSignalGroupsWithSecrets returns signal groups with per-user encrypted secrets for a connection.
func (r *ConnectionRepository) ListConnectionSignalGroupsWithSecrets(ctx context.Context, connectionID string, userID string) ([]models.SignalGroupWithSecret, error) {
	...
	for rows.Next() {
		...
		var group models.SignalGroupWithSecret
		if scanErr := rows.Scan(
			&group.ID,
			&group.RegionID,
			&group.SchoolID,
